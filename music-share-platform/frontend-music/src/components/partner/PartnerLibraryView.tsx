...
export function PartnerLibraryView() {
  ...
  const loadTracks = useCallback(async () => {
    setLoading(true);
    try {
      const res = await partnerAPI.getLibrary(searchParams);
  ...
  const handlePlay = async (track: LibraryTrack) => {
    ...
    try {
      ...
      const playableTracks = tracks.filter(t => t.isAssigned);
      ...
      usePlayerStore.setState({
        ...
        playlist: playableTracks.map(t => ({ id: t.id, title: t.title, artist: t.artist, album: t.album || undefined, duration: t.duration || undefined })),
        currentIndex: playableTracks.findIndex(t => t.id === track.id),
        isPlaying: true,
        isLoading: false,
        currentTime: 0
      });
      ...
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <div>
          ...
          <p className={cn("text-sm sm:text-base mt-0.5 sm:mt-1", isDark ? "text-white/50" : "text-gray-500")}>
            {pagination.total > 0 ? `총 ${pagination.total}개의 트랙` : '음악 트랙을 탐색하세요.'}
          </p>
        </div>
        <button
          onClick={loadTracks}
        ...
        {tracks.length === 0 && !loading ? (
          <div className="p-8 sm:p-16 text-center">
            ...
            <p className={cn("text-sm mt-1", isDark ? "text-white/50" : "text-gray-500")}>
              {activeFilterCount > 0 ? '다른 검색어나 필터를 시도해보세요.' : '등록된 트랙이 없습니다.'}
            </p>
            {activeFilterCount > 0 && (
              <button
                onClick={clearFilters}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        ...
                        disabled={(playerLoading && currentTrack?.id === track.id) || !track.isAssigned}
                        className={cn(
                      ...
                      <td className="px-6 py-4">
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex justify-end gap-1">
                          <button
                            ...
                            title={!track.isAssigned ? "권한 없음" : currentTrack?.id === track.id && isPlaying ? "일시정지" : "재생"}
                          >
                            {currentTrack?.id === track.id && playerLoading ? (
                              <Loader2 className="w-4 h-4 animate-spin" />
                            ) : currentTrack?.id === track.id && isPlaying ? (
                          ...
                          <button
                            ...
                            title={!track.isAssigned ? "권한 없음" : "다운로드"}
                          >
                            <Download className="w-4 h-4" />
                          </button>
                        </div>
